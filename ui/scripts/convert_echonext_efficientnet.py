"""Convert EfficientNetV2B0 EchoNext HFpEF/HFrEF model (.keras) to TensorFlow.js LayersModel format.
...
ZIP_PATH = os.path.join(ROOT, 'model-src', 'hfpef_hfref.zip')
...
def main() -> None:
    ...
    extract_dir = os.path.join(ROOT, 'model-src', '_hfpef_hfref_extracted')
    ...
    # Sanity: single forward pass on a random input matching (160, 160, 3).
    x = np.random.default_rng(0).standard_normal((1, 160, 160, 3)).astype(np.float32)
