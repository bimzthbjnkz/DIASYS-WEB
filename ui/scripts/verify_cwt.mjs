...
import { cwtMexh } from '../src/lib/cwtMexh.ts'

const ref = JSON.parse(readFileSync(new URL('./cwt_reference.json', import.meta.url), 'utf-8'))
const sig = new Float32Array(ref.sig)
const mag = ref.mag

const t0 = performance.now()
const out = cwtMexh(sig)
...
for (let si = 0; si < 32; si++) {
  for (let k = 0; k < 2500; k++) {
    const diff = Math.abs(out[si * 2500 + k] - mag[si][k])
    if (diff > worst) {
      worst = diff
